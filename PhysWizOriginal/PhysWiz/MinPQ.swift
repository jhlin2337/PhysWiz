    ...
    */
    ...
    mutating func insert(elem: T){
        if self.size+1 >= self.heap.count {
            self.heap.appendContentsOf(Array<T?>(count: self.size + 1, repeatedValue: nil))
        }
...
extension MinPQ {
    ...
    init(array: [T], compareFunc: (T, T) -> Bool) {
        ...
        self.heap.appendContentsOf(array.map({$0}))
        for var i = size/2; i >= 0; i -= 1 {
            self.sink(i)
        }
    }
    ...
    init(otherPQ: MinPQ<T>) {
        ...
        self.heap.appendContentsOf(otherPQ.heap)
    }
}
...
extension MinPQ : CustomStringConvertible {
